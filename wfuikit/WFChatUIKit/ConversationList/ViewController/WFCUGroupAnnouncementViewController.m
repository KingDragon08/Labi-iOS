...
- (void)viewDidLoad {
    ...
    if (self.announcement.author.length && self.announcement.text.length) {
        ...
        self.timeLabel.textColor = [UIColor colorWithRed:149/255.0 green:150/255.0 blue:151/255.0 alpha:1.0];
    ...
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(16, kStatusBarAndNavigationBarHeight + offset, self.view.bounds.size.width - 32, WFCCUtilities.onepxLine)];
    line.backgroundColor = WFCCUtilities.onePxLineColor;
    [self.view addSubview:line];
    
    CGFloat hintSize = 0;
    if (!self.isManager) {
        line = [[UIView alloc] initWithFrame:CGRectMake(70, self.view.bounds.size.height - kTabbarSafeBottomMargin - 22, self.view.bounds.size.width - 140, WFCCUtilities.onepxLine)];
        line.backgroundColor = WFCCUtilities.onePxLineColor;
        [self.view addSubview:line];
        
        UILabel *hint = [[UILabel alloc] initWithFrame:CGRectMake(100, self.view.bounds.size.height - kTabbarSafeBottomMargin - 30, self.view.bounds.size.width - 200, 16)];
        hint.backgroundColor = [UIColor whiteColor];
        hint.textAlignment = NSTextAlignmentCenter;
        hint.text = @"仅群主及群管理员可编辑";
        hint.textColor = [UIColor colorWithRed:165/255.0 green:166/255.0 blue:167/255.0 alpha:1.0];
