...
- (void)setSearchInfo:(WFCCConversationSearchInfo *)searchInfo {
    ...
    if (searchInfo.marchedCount > 1) {
        ...
        [attrStr addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithRed:29/255.0 green:214/255.0 blue:87/255.0 alpha:1.f] range:range];
...
- (void)setInfo:(WFCCConversationInfo *)info {
    ...
    //    } else {
    if (info.isTop) {
        [self.contentView setBackgroundColor:[WFCUConfigManager globalManager].topColor];
    } else {
...
- (UILabel *)digestView {
    if (!_digestView) {
        ...
        _digestView.textColor = [UIColor colorWithRed:135/255.0 green:136/255.0 blue:137/255.0 alpha:1.f];
...
- (UILabel *)timeView {
    if (!_timeView) {
        ...
        _timeView.textColor = [UIColor colorWithRed:161/255.0 green:163/255.0 blue:164/255.0 alpha:1.f];
...
- (UIView *)separator {
    if (!_separator) {
        CGFloat lineHeight =  WFCCUtilities.onepxLine;
        _separator = [[UIView alloc] initWithFrame:CGRectMake(76, 72 - lineHeight, [UIScreen mainScreen].bounds.size.width-76, lineHeight)];
        _separator.backgroundColor = [UIColor redColor];
        [_separator setBackgroundColor:[UIColor colorWithRed:207/255.0 green:208/255.0 blue:209/255.0 alpha:1.f]];
