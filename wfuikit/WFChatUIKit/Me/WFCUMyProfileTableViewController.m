...
- (void)viewDidLoad {
    ...
    self.tableView.backgroundColor = [UIColor colorWithRed:236/255.f green:237/255.f blue:238/255.f alpha:1.0f];
...
- (void)addLabel:(NSString *)titleStr onCell:(UITableViewCell *)cell isHeaderCell:(BOOL)isHeaderCell isLeft:(BOOL)left {
    ...
    if (isHeaderCell) {
        line.frame = CGRectMake(8, 86 - WFCCUtilities.onepxLine,width - 8 , WFCCUtilities.onepxLine);
        title = [[UILabel alloc] initWithFrame:CGRectMake(8, 9, 68, 68)];
    } else {
        if (left) {
            title = [[UILabel alloc] initWithFrame:CGRectMake(8, 7, 72, 36)];
        } else {
            title = [[UILabel alloc] initWithFrame:CGRectMake(88, 7, width - 108 - 28, 36)];
        }
        line.frame = CGRectMake(8, 50 - WFCCUtilities.onepxLine,width - 8 , WFCCUtilities.onepxLine);
    }
