...
@property (nonatomic, assign)BOOL singleChat;
@end

@implementation WFCUPluginBoardView
- (instancetype)initWithDelegate:(id<WFCUPluginBoardViewDelegate>)delegate withVoip:(BOOL)withWoip isSingleChat:(BOOL)single {
    CGFloat width = [UIScreen mainScreen].bounds.size.width-16;
    self = [super initWithFrame:CGRectMake(0, 0, width, PLUGIN_AREA_HEIGHT)];
    if (self) {
        self.delegate = delegate;
        self.hasVoip = withWoip;
        self.singleChat = single;
        ...
        if (self.hasVoip) {
            if (self.singleChat) {
                 _pluginItems = [@[
                                ...
                                [[PluginItem alloc] initWithTitle:@"红包" image:[UIImage imageNamed:@"ChatRomm_ToolPanel_Icon_Luckymoney"] tag:7],
                                [[PluginItem alloc] initWithTitle:@"转账" image:[UIImage imageNamed:@"ChatRomm_ToolPanel_Icon_Pay"] tag:8],
                            ] mutableCopy];
            } else {
                 _pluginItems = [@[
                                [[PluginItem alloc] initWithTitle:WFCString(@"Album") image:[UIImage imageNamed:@"chat_input_plugin_album"] tag:1],
                                ...
                                [[PluginItem alloc] initWithTitle:@"位置" image:[UIImage imageNamed:@"chat_input_plugin_location"] tag:3],
                #if WFCU_SUPPORT_VOIP
                                [[PluginItem alloc] initWithTitle:@"语音通话" image:[UIImage imageNamed:@"ChatRomm_ToolPanel_Icon_Multitalk"] tag:4],
                #endif
                                [[PluginItem alloc] initWithTitle:@"文件" image:[UIImage imageNamed:@"chat_input_plugin_file"] tag:5],
