...
//        [self addSubview:self.facePageControl];
        ...
        _sendBtn.backgroundColor = [UIColor colorWithRed:22/255.0 green:185/255.0 blue:77/255.0 alpha:1.0];
        ...
        self.collectionView.frame = CGRectMake(0, EMOJ_TAB_HEIGHT, self.frame.size.width, self.frame.size.height - EMOJ_TAB_HEIGHT);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (scrollView == self.collectionView) {
        int currentPage = ((scrollView.contentOffset.x / self.frame.size.width) + 0.2);
        if (self.selectedTableRow != currentPage) {
            self.selectedTableRow = currentPage;
        }
    } else if (scrollView == emojView) {
        UIWindow * window = [[[UIApplication sharedApplication] delegate] window];
        CGRect btnRect = [btnView convertRect:btnView.bounds toView:window];
        for (UIView *sub in emojView.subviews) {
            CGRect start =  [sub convertRect:sub.bounds toView:window];
            if (CGRectIntersectsRect(btnRect, start)) {
                [sub setHidden:YES];
                 NSLog(@"emoj#########");
            } else {
                [sub setHidden:NO];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (collectionView == self.collectionView) {
        if (indexPath.section == 0) {
            btnView = [UIView new];
            btnView.backgroundColor = [WFCUConfigManager globalManager].backgroudColor;
            btnView.frame = CGRectMake(self.frame.size.width - 140, self.frame.size.height - 100, 135, 50);
            [btnView addSubview:self.sendBtn];
            _sendBtn.frame = CGRectMake(btnView.frame.size.width - 60,0,56, 44);
            
            UIButton *delBack = [UIButton buttonWithType:UIButtonTypeCustom];
            [delBack setImage:[UIImage imageNamed:@"del_emoji_normal"] forState:UIControlStateNormal];
            [delBack setImage:[UIImage imageNamed:@"del_emoji_select"] forState:UIControlStateSelected];
            [delBack addTarget:self action:@selector(backFace) forControlEvents:UIControlEventTouchUpInside];
            [btnView addSubview:delBack];
            delBack.frame = CGRectMake(10,0, 56, 44);
            delBack.layer.cornerRadius = 3.0;
            delBack.backgroundColor = [UIColor whiteColor];
            [cell.contentView addSubview:btnView];
            
            UIView *bg = [self emojiView];
            bg.frame = cell.bounds;
            [cell.contentView insertSubview:bg belowSubview:btnView];
            
        } else {
            UIView *imageContent = [self picView:indexPath.section];
            [cell.contentView addSubview:imageContent];
            imageContent.frame = cell.bounds;
...
//            }
        ...
        if (indexPath.row == self.selectedTableRow) {
            ...
            cell.backgroundColor = [UIColor colorWithRed:242/255.0 green:244/255.0 blue:245/255.0 alpha:1.f];
...
- (UIView *) picView: (NSInteger)section {
    UIScrollView *sc = [UIScrollView new];
    UIView *lastView = nil;
    
    NSArray *paths = self.stickers[self.stickers.allKeys[section - 1]].stickerPaths;
    
//    int startPos = (int)indexPath.row * STICKER_COUNT_PAGE;
//    int endPos = (int)MIN(paths.count, startPos +  STICKER_COUNT_PAGE);
    for (int i = 0; i < paths.count; i++) {
        int cli = i/STICKER_COUNT_CLU;
        int col = i%STICKER_COUNT_CLU;
        
        CGFloat x = col * width/STICKER_COUNT_CLU;
        CGFloat y = cli * STICKER_ICON_SIZE + 10*(cli +1);
    ...
    sc.contentSize = CGSizeMake(self.frame.size.width, CGRectGetMaxY(lastView.frame) + 10);
    return sc;
}

- (UIView *) emojiView {
    if (emojView != nil) {
        emojView.delegate = nil;
    }
    emojView = [UIScrollView new];
    UIWindow * window = [[[UIApplication sharedApplication] delegate] window];
    CGRect btnRect = [btnView convertRect:btnView.bounds toView:window];
    UIView *last = nil;
    for (int i = 0; i < self.faceEmojiArray.count; i++) {
        int cli = i/FACE_COUNT_CLU;
        int col = i%FACE_COUNT_CLU;
        ...
        [emojView addSubview:faceButton];
        if (i == self.faceEmojiArray.count - 1) {
            last = faceButton;
        }
        
        CGRect start =  [faceButton convertRect:faceButton.bounds toView:window];
        if (CGRectIntersectsRect(btnRect, start)) {
            [faceButton setHidden:YES];
        } else {
            [faceButton setHidden:NO];
        }
    }
    emojView.contentSize = CGSizeMake(self.frame.size.width, CGRectGetMaxY(last.frame) + 40);
