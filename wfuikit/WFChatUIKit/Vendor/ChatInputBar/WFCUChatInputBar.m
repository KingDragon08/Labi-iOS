...
#define CHAT_VIDEO_COVER_MAX_WIDTH 480
...
@property (nonatomic, strong) CompressVideoManager *cManager;
...
- (instancetype)initWithParentView:(UIView *)parentView conversation:(WFCCConversation *)conversation delegate:(id<WFCUChatInputBarDelegate>)delegate {
    ...
    if (self) {
        ...
        self.cManager = [CompressVideoManager new];
    }
    return self;
}
...
- (void)initSubViews {
    ...
    line.frame = CGRectMake(0, 0, self.frame.size.width, WFCCUtilities.onepxLine);
    self.backgroundColor = [UIColor colorWithRed:242/255.0 green:244/255.0 blue:245/255.0 alpha:1.f];
    
    CGRect parentRect = self.bounds;
    self.voiceSwitchBtn = [[UIButton alloc] initWithFrame:CGRectMake(CHAT_INPUT_BAR_PADDING, CHAT_INPUT_BAR_PADDING, 0, 0)];
    ...
    self.pluginSwitchBtn = [[UIButton alloc] initWithFrame:CGRectMake(parentRect.size.width - CHAT_INPUT_BAR_HEIGHT + CHAT_INPUT_BAR_PADDING, CHAT_INPUT_BAR_PADDING, 0, 0)];
    ...
    self.emojSwitchBtn = [[UIButton alloc] initWithFrame:CGRectMake(parentRect.size.width - CHAT_INPUT_BAR_HEIGHT, CHAT_INPUT_BAR_PADDING, CHAT_INPUT_BAR_ICON_SIZE, CHAT_INPUT_BAR_ICON_SIZE)];
    ...
    self.textInputView = [[UITextView alloc] initWithFrame:CGRectMake(CHAT_INPUT_BAR_HEIGHT - 40, CHAT_INPUT_BAR_PADDING, parentRect.size.width - CHAT_INPUT_BAR_HEIGHT - CHAT_INPUT_BAR_HEIGHT - CHAT_INPUT_BAR_HEIGHT + CHAT_INPUT_BAR_PADDING + CHAT_INPUT_BAR_ICON_SIZE + 40, CHAT_INPUT_BAR_ICON_SIZE)];
    ...
    [self.voiceInputBtn setTitle:@"按住 说话" forState:UIControlStateNormal];
    [self.voiceInputBtn setTitle:@"松开 结束" forState:UIControlStateHighlighted];
    self.voiceInputBtn.titleLabel.font = [UIFont boldSystemFontOfSize:15.0];
    [self.voiceInputBtn setTitleColor:WFCCUtilities.grayTextColor forState:UIControlStateNormal];
    [self.voiceInputBtn setTitleColor:[UIColor blackColor] forState:UIControlStateHighlighted];
    [self.voiceInputBtn setBackgroundColor:[UIColor whiteColor]];
    ...
    [self.voiceInputBtn setBackgroundImage:[UIImage new] forState:UIControlStateNormal];
    [self.voiceInputBtn setBackgroundImage:[UIImage imageNamed:@"fts_gray_btn_HL"] forState:UIControlStateHighlighted];
    ...
    line2.frame = CGRectMake(0, 0, self.frame.size.height-WFCCUtilities.onepxLine, WFCCUtilities.onepxLine);
}
...
- (UIView *)pluginInputView {
    if (!_pluginInputView) {
        _pluginInputView = [[WFCUPluginBoardView alloc] initWithDelegate:self withVoip:(self.conversation.type == Single_Type || self.conversation.type == Group_Type) isSingleChat: (self.conversation.type == Single_Type)];
    }
...
- (void)changeTextViewHeight:(CGFloat)height needUpdateText:(BOOL)needUpdateText updateRange:(NSRange)range {
    ...
    CGFloat diff = 0;
    if (height <= 38.f) {
        tvFrame.size.height = 38.f;
        diff = (55.f - baseFrame.size.height);
        baseFrame.size.height = 55.f;
    } else if (height > 38.f && height < 57.f) {
        tvFrame.size.height = 57.f;
        diff = (73.f - baseFrame.size.height);
        baseFrame.size.height = 73.f;
    } else {
        tvFrame.size.height = 72.f;
        diff = (88.f - baseFrame.size.height);
        baseFrame.size.height = 88.f;
    }
...
- (void)export:(AVURLAsset *)videoAsset success:(void(^)(NSString *path))completed {
    /// 资源
    ...
    if ([presets containsObject:AVAssetExportPresetHighestQuality]) {
        ...
        NSString *outputPath = [videoPath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.mp4",videoName]];
...
//        videoCom.renderSize = vSize;
//        session.videoComposition = videoCom;
        ...
        [session exportAsynchronouslyWithCompletionHandler:^(void) {
            switch (session.status) {
                ...
                case AVAssetExportSessionStatusCompleted: {
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if (completed) {
                            NSLog(@"size is %ld", [[NSData dataWithContentsOfFile:outputPath] length] / 1024);
...
- (CGSize)resizeSize:(CGSize)originSize {
    if (originSize.width > CHAT_VIDEO_COVER_MAX_WIDTH || originSize.height > CHAT_VIDEO_COVER_MAX_WIDTH) {
        if (originSize.width > originSize.height){
            CGFloat mWidth = originSize.width;
            originSize.width = CHAT_VIDEO_COVER_MAX_WIDTH;
            originSize.height = CHAT_VIDEO_COVER_MAX_WIDTH/ mWidth * originSize.height;
        } else {
            CGFloat mHeight = originSize.height;
            originSize.height = CHAT_VIDEO_COVER_MAX_WIDTH;
            originSize.width = CHAT_VIDEO_COVER_MAX_WIDTH / mHeight * (originSize.width);
        }
    }
    return originSize;
}
    ...
    if (itemTag == 1) {
        ...
        [imagePickerVc setDidFinishPickingVideoHandle:^(UIImage *coverImage, PHAsset *asset) {
            ...
            [[PHImageManager defaultManager] requestAVAssetForVideo:asset options:options resultHandler:^(AVAsset* avasset, AVAudioMix* audioMix, NSDictionary* info){
               [self export:(AVURLAsset *)avasset success:^(NSString *path) {
                   CGSize targetSize = [self resizeSize:coverImage.size];
                   ...
                   [coverImage drawInRect:thumbnailRect];

                   newImage = UIGraphicsGetImageFromCurrentImageContext();
                   UIGraphicsEndImageContext();
                  
                     [self.delegate videoDidCapture:path thumbnail: newImage duration: asset.duration];
                }];
            }];
        }];
        ...
        [imagePickerVc setDidFinishPickingPhotosHandle:^(NSArray<UIImage *> *photos, NSArray *assets, BOOL isSelectOriginalPhoto) {
            if (photos.count > 0) {
                for (UIImage *phote in photos) {
                    [self.delegate imageDidCapture:phote];
                }
            }
        }];
        [navi presentViewController:imagePickerVc animated:YES completion:nil];
        [self checkAndAlertCameraAccessRight];
    } else if(itemTag == 2) {
#if TARGET_IPHONE_SIMULATOR
        [self makeToast:@"模拟器不支持相机" duration:1 position:CSToastPositionCenter];
        UIImagePickerController *picker = [[WQImagePickerController alloc] init];
        ...
        /// 红包
        if ([self.delegate respondsToSelector:@selector(didSelectLuckyMoney)]) {
            [self.delegate didSelectLuckyMoney];
        }
    } else if(itemTag == 8) {
        /// 转账
        if ([self.delegate respondsToSelector:@selector(didSelectTransferAccounts)]) {
            [self.delegate didSelectTransferAccounts];
        }
    }
}
