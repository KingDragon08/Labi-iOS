...
#define Portrait_Padding_Left 12
#define Portrait_Padding_Right 12
...
+ (CGFloat)bubbleWidth {
    return ([UIScreen mainScreen].bounds.size.width - (Portrait_Size + Portrait_Padding_Left + Portrait_Padding_Right) * 2);
}
...
//      }
    ...
    self.contentArea.frame = CGRectMake(Bubble_Padding_Arraw, Client_Bubble_Top_Padding, size.width, size.height);
      ...
      UIImage *image = self.bubbleView.image;
      self.bubbleView.image = [self.bubbleView.image resizableImageWithCapInsets:UIEdgeInsetsMake(image.size.height * 0.8, image.size.width * 0.8,image.size.height * 0.2, image.size.width * 0.2)];
  }
