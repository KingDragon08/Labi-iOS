...
- (void)setMoneyMessageHasAccept:(long)messageId {
    WFCCMessage *message = [self getMessage:messageId];
    if (!message || ![message.content isKindOfClass:[WFCCLuckMoneyMessage class]] || message.direction == MessageDirection_Send) {
        return;
    }
    mars::stn::MessageDB::Instance()->updateMessageStatus(messageId, mars::stn::Message_Status_Accept);
}
